...
module("Integration | Component | cd-icon-button", function (hooks) {
  ...
  test("it renders uikit icon buttons", async function (assert) {
    await render(hbs`<CdIconButton @icon="plus" @onClick={{this.onClick}} />`);
  ...
  test("it renders icon buttons from svg-jar", async function (assert) {
    await render(
      hbs`<CdIconButton @icon="add-outline" @fromSvgJar={{true}} @onClick={{this.onClick}} />`
    );
